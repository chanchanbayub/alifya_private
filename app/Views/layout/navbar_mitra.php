 <!-- ======= Sidebar ======= -->
 <aside id="sidebar" class="sidebar">

     <ul class="sidebar-nav" id="sidebar-nav">

         <li class="nav-item">
             <a class="nav-link" href="/mitra_pengajar/dashboard">
                 <i class="bi bi-grid"></i>
                 <span>Dashboard</span>
             </a>
         </li><!-- End Dashboard Nav -->


         <li class="nav-item">
             <a class="nav-link collapsed" data-bs-target="#icons-nav" data-bs-toggle="collapse" href="#">
                 <i class="bi bi-book"></i><span>Sumber Links</span><i class="bi bi-chevron-down ms-auto"></i>
             </a>
             <ul id="icons-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                 <li>
                     <a href="/mitra_pengajar/alat_pengajaran">
                         <i class="bi bi-circle"></i><span>Alat Pengejaran</span>
                     </a>
                 </li>

             </ul>
         </li><!-- End Icons Nav -->

         <li class="nav-heading">PEMBIMBING</li>
         <li class="nav-item">
             <a class="nav-link collapsed" data-bs-target="#pembimbing-nav" data-bs-toggle="collapse" href="#">
                 <i class="bi-person-check"></i><span>Sistem Pembimbing</span><i class="bi bi-chevron-down ms-auto"></i>
             </a>
             <ul id="pembimbing-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                 <li>
                     <a href="/mitra_pengajar/peserta_ahl">
                         <i class="bi bi-circle"></i><span>Peserta Didik AHL</span>
                     </a>
                 </li>
                 <li>
                     <a href="/mitra_pengajar/data_murid/peserta_didik">
                         <i class="bi bi-circle"></i><span>Peserta Didik Private</span>
                     </a>
                 </li>
             </ul>
         </li>


         <li class="nav-heading">ALIFYA HOME LEARNING</li>

         <li class="nav-item">
             <a class="nav-link collapsed" data-bs-target="#ahl-nav" data-bs-toggle="collapse" href="#">
                 <i class="bi bi-house"></i><span>Alifya Home Learning</span><i class="bi bi-chevron-down ms-auto"></i>
             </a>
             <ul id="ahl-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                 <li>
                     <a href="/mitra_pengajar/peserta_ahl">
                         <i class="bi bi-circle"></i><span>Peserta Didik AHL</span>
                     </a>
                 </li>
                 <li>
                     <a href="/mitra_pengajar/presensi_ahl_mitra">
                         <i class=" bi bi-circle"></i><span>Presensi AHL</span>
                     </a>
                 </li>
             </ul>
         </li>

         <li class="nav-heading">ALIFYA PRIVATE</li>

         <li class="nav-item">
             <a class="nav-link collapsed" data-bs-target="#jadwal-nav" data-bs-toggle="collapse" href="#">
                 <i class="bi bi-calendar"></i><span>Jadwal Mitra</span><i class="bi bi-chevron-down ms-auto"></i>
             </a>
             <ul id="jadwal-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                 <li>
                     <a href="/mitra_pengajar/jadwal_tetap">
                         <i class="bi bi-circle"></i><span>Jadwal Pengajar Mitra</span>
                     </a>
                 </li>

             </ul>
         </li>

         <li class="nav-item">
             <a class="nav-link collapsed" data-bs-target="#pengajar-nav" data-bs-toggle="collapse" href="#">
                 <i class="bi bi-person-check"></i><span>Mitra Pengajar</span><i class="bi bi-chevron-down ms-auto"></i>
             </a>
             <ul id="pengajar-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                 <li>
                     <a href="/mitra_pengajar/data_pengajar">
                         <i class="bi bi-circle"></i><span>Mitra Pengajar</span>
                     </a>
                 </li>

             </ul>
         </li>

         <li class="nav-item">
             <a class="nav-link collapsed" data-bs-target="#grup-nav" data-bs-toggle="collapse" href="#">
                 <i class="bi bi-file-earmark-person"></i><span>Kelompok Belajar</span><i class="bi bi-chevron-down ms-auto"></i>
             </a>
             <ul id="grup-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                 <li>
                     <a href="/mitra_pengajar/kelompok">
                         <i class="bi bi-circle"></i><span>Kelompok Pengajar</span>
                     </a>
                 </li>
                 <li>
                     <a href="/mitra_pengajar/data_murid">
                         <i class="bi bi-circle"></i><span>Peserta Didik</span>
                     </a>
                 </li>

             </ul>
         </li>

         <!-- <li class="nav-item">
             <a class="nav-link collapsed" data-bs-target="#media-nav" data-bs-toggle="collapse" href="#">
                 <i class="bi bi-cash-stack"></i><span>Harga Media Belajar</span><i class="bi bi-chevron-down ms-auto"></i>
             </a>
             <ul id="media-nav" class="nav-content collapse" data-bs-parent="#sidebar-nav">
                 <li>
                     <a href="/mitra_pengajar/media_belajar">
                         <i class="bi bi-circle"></i><span>Harga Media Belajar</span>
                     </a>
                 </li>
             </ul>
         </li> -->

         <li class="nav-item">
             <a class="nav-link collapsed" data-bs-target="#presensi-nav" data-bs-toggle="collapse" href="#">
                 <i class="bi bi-fingerprint"></i><span>Presensi Mitra</span><i class="bi bi-chevron-down ms-auto"></i>
             </a>
             <ul id="presensi-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                 <li>
                     <a href="/mitra_pengajar/presensi">
                         <i class="bi bi-circle"></i><span>Presensi Mitra</span>
                     </a>
                 </li>
                 <li>
                     <a href="/mitra_pengajar/absensi">
                         <i class="bi bi-circle"></i><span>Absensi Mitra</span>
                     </a>
                 </li>

             </ul>
         </li>

         <li class="nav-heading">Konfirmasi Invoice</li>

         <li class="nav-item">
             <a class="nav-link collapsed" data-bs-target="#konfirmasi-nav" data-bs-toggle="collapse" href="#">
                 <i class="bi bi-patch-check"></i><span>Konfirmasi Invoice</span><i class="bi bi-chevron-down ms-auto"></i>
             </a>
             <ul id="konfirmasi-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                 <li>
                     <a href="/mitra_pengajar/invoice">
                         <i class="bi bi-circle"></i><span>Konfirmasi Invoice</span>
                     </a>
                 </li>

             </ul>
         </li>
     </ul>

 </aside><!-- End Sidebar-->