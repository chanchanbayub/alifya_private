                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($status_murid as $status_murid) : ?>
                                        <tr>
                                            ...
                                            <td><?= $status_murid->status_murid ?></td>
                                            <td>
                                                <button class="btn btn-sm btn-outline-warning" id="edit" data-bs-toggle="modal" data-bs-target="#editModal" data-id="<?= $status_murid->id ?>" type="button">
                                                    <i class="bi bi-pencil-square"></i>
                                                </button>
                                                <button class="btn btn-sm btn-outline-danger" id="delete" data-bs-toggle="modal" data-bs-target="#deleteModal" data-id="<?= $status_murid->id ?>" type="button">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="add_form">
                    <?= csrf_field(); ?>
                    <div class="mb-3">
                        <label for="status_murid" class="col-form-label">Status Murid :</label>
                        <input type="text" class="form-control" id="status_murid" name="status_murid" placeholder="cth: aktif">
            ...
            <div class="modal-body">
                <form id="edit_form" autocomplete="off">
                    ...
                    <div class="form-group">
                        ...
                        <label for="status_murid_edit" class="col-form-label"><?= $title ?> :</label>
                        <input type="text" class="form-control" id="status_murid_edit" name="status_murid">
...
<script>
    $(document).ready(function(e) {
        $("#add_form").submit(function(e) {
            ...
            let status_murid = $("#status_murid").val();

            $.ajax({
                url: '/admin/status_murid/insert',
                method: 'post',
                dataType: 'JSON',
                data: {
                ...
                success: function(response) {
                    ...
                    if (response.error) {
                        if (response.error.status_murid) {
                            $("#status_murid").addClass('is-invalid');
                            $(".error-status").html(response.error.status_murid);
                        } else {
                            $("#status_murid").removeClass('is-invalid');
                            $(".error-status").html('');
                        }
    ...
    $(document).on('click', "#edit", function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#status_murid_edit").val(response.status_murid);
    ...
    $("#edit_form").submit(function(e) {
        e.preventDefault();
        let id = $('#id_edit').val();
        let status_murid = $('#status_murid_edit').val();

        $.ajax({
            url: '/admin/status_murid/update',
            method: 'post',
            dataType: 'JSON',
            data: {
                id: id,
            ...
            success: function(response) {
                ...
                if (response.error) {
                    if (response.error.status_murid) {
                        $("#status_murid_edit").addClass('is-invalid');
                        $(".error-status-edit").html(response.error.status_murid);
                    } else {
                        $("#status_murid_edit").removeClass('is-invalid');
