...
use App\Models\Mitra\JadwalTetapModel;
use App\Models\Mitra\KelompokBelajarModel;
use App\Models\Mitra\PengajarModel as MitraPengajarModel;
...
class JadwalTetapController extends BaseController
{
    ...
    {
        $this->pengajarModel = new MitraPengajarModel();
    ...
    {
        // dd(session()->get());
        $mitra = $this->pengajarModel->getMitraPengajarWithId(session()->get('mitra_pengajar_id'));

        $jadwal_tetap = $this->jadwalTetapModel->getJadwalWithMitra(session()->get('mitra_pengajar_id'));

        // dd(session()->get());
        $data = [
            'title' => 'Jadwal Tetap Mitra',
            'hari_belajar' => $this->hariBelajarModel->getHari(),
            'mitra_pengajar' => $mitra,
            'hari_belajar' => $this->hariBelajarModel->getHari(),
            'jadwal_tetap' => $jadwal_tetap
        ];

        return view('mitra/jadwal_tetap_v', $data);
    }
