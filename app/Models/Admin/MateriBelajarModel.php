...
class MateriBelajarModel extends Model
{
    ...
    public function getMateriBelajarWithProgramBelajarId($program_belajar_id)
    {
        return $this->table($this->table)
            ->select("materi_belajar_table.id, materi_belajar_table.level, materi_belajar_table.program_belajar_id")
            // ->join('program_belajar_table', 'program_belajar_table.id = materi_belajar_table.program_belajar_id')
            ->where(["materi_belajar_table.program_belajar_id" => $program_belajar_id])
            ->orderBy('materi_belajar_table.id desc')
            ->get()->getResultObject();
    }
}
