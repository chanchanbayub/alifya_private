...
class KuisionerKreativitasModel extends Model
{
    ...
    public function getKuisionerKreativitas($pembimbing_id, $mitra_pengajar_id, $bulan, $tahun)
    {
        return $this->table($this->table)
            ->select('kuisioner_kreativitas_table.id , pembimbing_id, bulan, tahun, data_pengajar_table.nama_lengkap, skala_nilai_table.bobot as kreativitas, skala_nilai_table.kategori_apr_id')
    ...
    public function getKuisionerKreativitasData($pembimbing_id, $mitra_pengajar_id, $bulan, $tahun)
    {
        return $this->table($this->table)
            ->select('kuisioner_kreativitas_table.id , pembimbing_id, bulan, tahun, data_pengajar_table.nama_lengkap, skala_nilai_table.bobot as kreativitas, skala_nilai_table.kategori_apr_id, skala_nilai_table.keterangan, skala_nilai_table.nilai')
            ->join('data_pengajar_table ', 'data_pengajar_table.id = kuisioner_kreativitas_table.mitra_pengajar_id')
            ->join('skala_nilai_table', 'skala_nilai_table.id = kuisioner_kreativitas_table.kreativitas')
            ->where(["pembimbing_id" => $pembimbing_id])
            ->where(["mitra_pengajar_id" => $mitra_pengajar_id])
