...
class HargaModel extends Model
{
    ...
    {
        return $this->table($this->table)
            ->select('harga_table.id, harga_table.peserta_didik_id, harga_table.harga,  data_murid_table.nama_lengkap_anak, harga_table.bulan, data_pengajar_table.nama_lengkap')
            ->join('data_murid_table', 'data_murid_table.id = harga_table.peserta_didik_id')
